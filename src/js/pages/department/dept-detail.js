...
class DeptDetail extends LitElement {
    ...
    editDept() {
        return html`
            ...
            <div class="w3-row" style="padding-top: 10px">
                <input class="w3-check" type="checkbox" .checked=${this.isActive} } 
                    @click=${() => { this.isActive = !this.isActive }}>
                <label>${this.isActive ? 'Đang ' : 'Dừng '} sử dụng</label>
            </div>
            <div class="w3-row" style="padding-top: 10px">
                <button class="w3-button w3-teal w3-round-xlarge" @click=${this.saveEdit}>Save</button>
                <button class="w3-button w3-khaki w3-round-xlarge" @click=${this.cancelDetail}>Cancel</button>
            </div>
        `
    }
    ...
    newDept() {
        return html`
            ...
            <div class="w3-row" style="padding-top: 10px">
                <input class="w3-check" type="checkbox" .checked=${this.isRoot} 
                     @click=${() => { this.isRoot = !this.isRoot }}
                >
                <label>Chuyển thành phòng ban chính</label>
            </div>
            <div class="w3-row" style="padding-top: 10px">
                <button class="w3-button w3-teal w3-round-xlarge" @click=${this.saveNew}>Save</button>
                <button class="w3-button w3-khaki w3-round-xlarge" @click=${this.cancelDetail}>Cancel</button>
            </div>
        `
    }
    ...
    render() {
        ...
        ${(this.mode) ? ((this.mode === 'edit') ? html`${this.editDept()}` : html`${this.newDept()}`) : ''}
        `
    }
}
